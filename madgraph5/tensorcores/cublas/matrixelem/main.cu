...
TTYPE mult_native_host(TTYPE *cf, std::complex<TTYPE> *jamp) {
  ...
  for( int icol = 0; icol < ncolor; icol++ ) {
    ...
    deltaME += ( ztemp.real() * jamp[icol].real()  + ztemp.imag() * jamp[icol].imag() ) ; // / denom[icol];
  }
  return deltaME;
}
...
__global__
void mult_native_device(const TTYPE *cf, const TTYPE *jampr, const TTYPE *jampi, TTYPE *deltaME) {
  int ncolor = 24;
  *deltaME = 0;
  for( int icol = 0; icol < ncolor; icol++ ) {
...
int mult_cublas(cublasHandle_t handle, const TTYPE *d_A, const TTYPE *d_B, TTYPE *d_C, TTYPE *d_y, TTYPE *h_y, int dsize, float& time,  const TTYPE* d_Bt = 0) {
  ...
  t.Start();
  cublas_status = CUB_SYMV(handle, uplo, m, &alpha, d_A, lda, d_B, 1, &beta, d_C, 1);
...
int main() {
  ...
  int dsize = sizeof(TTYPE),
      ...
      mult_status = 0;
  const TTYPE
    *h_A = (TTYPE *)malloc(msize),                   // color matrix
  ...
  mult_status = mult_cublas(handle, d_A, d_Bi, d_C, d_y, h_y, dsize, tmptime, d_Bt);
  time += tmptime;
  me += *h_y;
  ...
  //cuda_status = cudaMemcpy((void*) d_jamp, jamp, vsize * 2, cudaMemcpyHostToDevice);
  time = 0.;
  t.Start();
  mult_native_device<<<1,1>>>(d_A, d_Br, d_Bi, d_y);
  cuda_status = cudaMemcpy(h_y, d_y, dsize, cudaMemcpyDeviceToHost);
  std::cout << "native device: " << *h_y << ", " << t.GetDuration() << std::endl;
